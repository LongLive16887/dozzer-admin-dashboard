...
export const CategoriesList = () => {
  ...
  if(typeof dataCategories?.data === "object" && dataCategories.data.results.length <= 0)
    ...
    <div className="flex flex-1 flex-col gap-4 p-4">
      <div className="rounded-xl bg-muted/50 p-4">
        <Table className="">
          ...
          <TableBody>
          {typeof dataCategories?.data === "object" && dataCategories.data.results.length > 0 ? (dataCategories.data.results.map((category) => (
              <Category
                  key={category.id}
                  category={category}
