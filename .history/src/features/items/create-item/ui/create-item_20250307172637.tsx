...
export const CreateItem = () => {
  ...
  const handleChange = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    setFormData({
      ...formData,
      [name]: ["vizov_price", "smenu_price", "smenu_duration", "hourly_price"].includes(name) ? Number(value) : value,
    });
  };
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const response = await createItem(itemData).unwrap();
      ...
      <DialogContent className="sm:max-w-[500px] sm:max-h-[600px] overflow-scroll">
        ...
        <form onSubmit={handleSubmit}>
          <div className="grid gap-4 py-4">
            ...
            <Input name="begin_date" value={formData.begin_date} onChange={handleChange} placeholder="Begin Date (YYYY-MM-DD HH:mm)" required />
  ...
  const handleFileChange = (e: React.ChangeEvent<HTMLInputElement>) => {
    if (e.target.files) {
      const filesArray = Array.from(e.target.files).map((file) => ({
        file_id: URL.createObjectURL(file), // Temporary URL for preview
        name: file.name,
        file_path: "", // This should be updated after upload
        size: file.size,
        extension: file.name.split(".").pop(),
        is_main: formData.photos.length === 0,
      }));
    ...
    Object.entries(formData).forEach(([key, value]) => {
      if (key === "photos") {
        formData.photos.forEach((photo, index) => {
          itemData.append(`photos[${index}]`, photo.file);
        });
      } else {
        itemData.append(key, value as string);
      }
    });
    itemData.append("category_id", categoryId || "");
    itemData.append("sub_category_id", subcategoryId || "");
