...
export const CreateItem = () => {
  ...
  const getCurrentDateTime = () => {
    const now = new Date();
    now.setTime(now.getTime() + 5 * 60 * 60 * 1000); // Чёткое добавление 5 часов
  ...
  const [formData, setFormData] = useState({
    ...
    smenu_duration: "",
    hourly_price: "",
    begin_date: getCurrentDateTime(), // Теперь вызываем один раз при первом рендере
    description_uz: "",
    description_ru: "",
    base_id: "",
  });
  ...
  useEffect(() => {
    if (showModal) {
      setFormData((prev) => ({
        ...prev,
        begin_date: getCurrentDateTime(), // Устанавливаем дату только при ОТКРЫТИИ
      }));
    }
  }, [showModal]);
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      setFormData({
        ...
        description_ru: "",
        base_id: "",
        begin_date: getCurrentDateTime(), // Устанавливаем новую дату после успешного добавления
      });
    } catch (err) {
      ...
      <DialogContent className="sm:max-w-[500px] sm:max-h-[600px] overflow-scroll">
        ...
        <form onSubmit={handleSubmit}>
          <div className="grid gap-4 py-4">
            ...
            <Input name="begin_date" value={formData.begin_date} disabled />
